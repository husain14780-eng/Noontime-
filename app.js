...
const state = {
  ...
  refreshToken: localStorage.getItem("noor_owner_refresh") || "",
  editingProduct: null,
  newOrderCount: 0,
  orderPoll: null,
...
function persistCart(){
  localStorage.setItem("cod_store_cart", JSON.stringify(state.cart));
  renderCartCount();
}
...
function setButtonBusy(id,busy,label){
  ...
  if(label!==undefined){
    ...
    b.textContent=busy?label:b.dataset.originalLabel;
  }
}
...
/* Refresh an expired owner session using Supabase refresh_token. */
async function refreshOwnerSession(){
  ...
  try {
    ...
    localStorage.setItem("noor_owner_token",state.accessToken);
    localStorage.setItem("noor_owner_refresh",state.refreshToken);

    state.owner=data.user || state.owner;
...
async function request(url, options={}){
  ...
  if(!res.ok){
    ...
    /* If an owner JWT expired, silently refresh it and retry once. */
    if(
      res.status === 401 &&
      !options.skipRefresh &&
      token &&
...
function normalizeCartAgainstProducts(){
  ...
  state.cart = state.cart.map(item=>{
    ...
    if(!p || Number(p.stock)<=0 || !p.is_active){
      return null;
    }
...
function productImage(product){
  if(product.image_url){
    return `<img class="product-image" src="${escapeHtml(product.image_url)}" alt="${escapeHtml(product.name)}" loading="lazy">`;
  }

  return `<div class="product-placeholder">NO IMAGE</div>`;
}

/* Public product loading ALWAYS uses no owner JWT. */
...
function changeQty(productId,delta){
  ...
  item.quantity=Math.max(
    0,
    Math.min(max,item.quantity+delta)
  );
...
async function placeOrder(form){
  ...
  try {
    ...
    toast(
      `Order #${String(orderId).slice(0,8).toUpperCase()} received — Cash on Delivery.`
    );
  ...
  try {
    ...
    if(state.lastOrderIds.size){
      ...
      const fresh=orders.filter(
        o=>!state.lastOrderIds.has(o.id)
      );

      if(fresh.length){
        state.newOrderCount+=fresh.length;
        updateNewBadge();
      }
    }
    ...
    $("ordersList").innerHTML=orders.map(order=>{
      ...
      const items=(order.order_items||[])
        .map(i =>
          `${escapeHtml(i.product_name)} × ${i.quantity} — ${money(Number(i.unit_price)*Number(i.quantity))}`
        )
        .join("<br>");
...
async function loadAdminProducts(){
  try {
    ...
    $("adminProductList").innerHTML=(data||[])
      .map(p => {
...
function openProductForm(product=null){
  ...
  $("productForm").elements.is_active.value=
    String(product?.is_active??true);
...
async function saveProduct(form){
  ...
  let image_url=
    state.editingProduct?.image_url||null;
  ...
  if(state.editingProduct){
    ...
    await request(
      `${API}/products?id=eq.${encodeURIComponent(state.editingProduct.id)}`,
      {
        method:"PATCH",
        headers:{
          "Content-Type":"application/json",
...
function updateNewBadge(){
  ...
  if(state.newOrderCount>0){
    ...
    b.textContent=state.newOrderCount;
    show("newOrderBadge");
...
function bindEvents(){
  ...
  document.addEventListener(
    ...
    async (e)=>{
      ...
      if(remove){
        ...
        state.cart=state.cart.filter(
          i=>i.product_id!==remove.dataset.remove
        );
      ...
      if(edit){
        ...
        try {
          ...
          const data=await request(
            `${API}/products?select=*&id=eq.${encodeURIComponent(edit.dataset.editProduct)}&limit=1`
          );

          if(data?.[0]){
            openProductForm(data[0]);
          }
      ...
      const inlineRefresh=e.target.closest(
        "#refreshOrdersInline"
      );
  ...
  document
    .querySelectorAll("[data-category-jump]")
    .forEach(btn=>
      btn.addEventListener(
        "click",
        ()=>{
          const cat=
            btn.dataset.categoryJump;
